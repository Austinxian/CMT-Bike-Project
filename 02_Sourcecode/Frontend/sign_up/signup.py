...
class signup(tk.Frame):
    def __init__(self, master):
        ...
        pwentry.pack(side=tk.RIGHT)

        self.label2text = tk.Frame(self)
        self.label2text.pack(fill=tk.X, padx=styleDict["xPadding"], pady=styleDict["yPadding"])
        text2 = tk.Label(self.label2text,
                         text='Password should contain at least 8 characters, including alphanumeric and a symbol.',
                         font=("Arial, 12"))
        ...
        exdate1entry1.pack(side=tk.LEFT, padx=15)
        ...
        exdate1entry2.pack(side=tk.LEFT, padx=15)
        ...
        cvventry.pack(side=tk.LEFT, padx=85)

        self.signup = tk.Frame(self)
        self.signup.pack(fill=tk.X, padx=styleDict["xPadding"], pady=styleDict["yPadding"])
        spbutton = tk.Button(self.signup, text='Sign Up', font=("Arial, 25"), width=10, height=2,
                             command=self.signupcheck)
        spbutton.pack(pady=10)
        self.canceltext = tk.Frame(self)
        self.canceltext.pack(fill=tk.X, padx=styleDict["xPadding"], pady=styleDict["yPadding"])
        cancel = tk.Label(self.canceltext, text='cancel', font=("Arial", "15", "underline"), fg="blue")
        cancel.bind("<Button-1>", self.restart_program)
        cancel.pack()
    ...
    def signupcheck(self):
        ...
        while (True):
            if (self.password.get() == self.cfpassword.get()):
                ...
                sql1 = sql.format(self.name.get(), self.email.get(), self.phone.get(), self.username.get(),
                                  self.password.get(), self.cardno.get(), self.exdate, self.cvv.get())
                print(sql1)
